...
def test_validate_lmstudio_url_rejects_missing_scheme() -> None:
    """A relative URL is caught up-front before httpx is invoked."""
    with pytest.raises(SystemExit):
        ai_module._validate_lmstudio_url("ftp:///models")  # noqa: SLF001


def test_validate_lmstudio_url_rejects_missing_host() -> None:
    """A URL with no netloc never reaches the network."""
    with pytest.raises(SystemExit):
        ai_module._validate_lmstudio_url("http:///models")  # noqa: SLF001
